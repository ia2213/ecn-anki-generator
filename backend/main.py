...
import io, re, os, math, uuid, shutil, subprocess, sys, tempfile
...
app = FastAPI(title="ECN Anki Generator API", version="3.0.0")
...
def extract_topic_from_pdf(elements):
    """Extraire le titre/sujet principal du PDF pour OpenDraft."""
    for elem in elements:
...
def ensure_opendraft():
    """Cloner OpenDraft si pas encore présent."""
    if not OPENDRAFT_DIR.exists():
        subprocess.run(
            ["git", "clone", "--depth=1", "https://github.com/federicodeponte/opendraft.git", str(OPENDRAFT_DIR)],
            check=True, capture_output=True, timeout=120
        )
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-r", str(OPENDRAFT_DIR / "requirements.txt"), "-q"],
            check=True, capture_output=True, timeout=300
        )

def run_opendraft_background(draft_id: str, topic: str, language: str, level: str):
    """Générer le draft académique en background."""
    ...
    try:
        ensure_opendraft()
        # Ajouter opendraft au path Python
...
# ── Routes ─────────────────────────────────────────────────────────────────
@app.get("/")
def root(): return {"message": "ECN Anki Generator API v3", "status": "running", "max_file_mb": 200}
...
async def parse_pdf(
    ...
    if len(content) > MAX_FILE_SIZE:
        raise HTTPException(status_code=413, detail=f"Fichier trop volumineux (max 200MB)")
    try:
        ...
        if generate_academic_draft:
            ...
            background_tasks.add_task(run_opendraft_background, draft_id, draft_topic, draft_language, draft_level)
...
async def create_study_plan(payload: dict):
    ...
    if total_cards <= 0: raise HTTPException(status_code=400, detail="total_cards doit être > 0")
